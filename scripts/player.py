...
class Player(pygame.sprite.Sprite):
  ...
  def import_assets(self):
    ...
    for animation in self.animations.keys():
      full_path = f"assets/graphics/character/{animation}" 
  ...
  def input(self):
    ...
    if keys[pygame.K_RETURN]:
        collide_interaction_sprite = pygame.sprite.spritecollide(self, self.interaction, False)
  ...
  def collect_items(self):
    for sprite in self.groups()[0].sprites():
      if getattr(sprite, 'collectible', False) and self.hitbox.colliderect(sprite.rect):
        sprite.kill()
